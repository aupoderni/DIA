...
def discretize_spectrum(spectrum, mass_proton, bin_width, bin_offset):
    ...
    bins = np.zeros(int(2000.0/bin_width), dtype='float64')
    for peak_idx, peak in enumerate(peak_bins):
        if peak < (int(2000.0/bin_width)) and bins[peak] < peaks[1][peak_idx]: #peak_bins[-1] + 1
...
def undiscretize_spectrum(bins, mass_proton, bin_width, bin_offset):
    mz = []
    intensity = []
    bins = [0 if el < 0.1 else el for el in bins]
